...
const endpoint=process.env.TEST_SERVER_URL||'http://localhost:2567';
...
}finally{await Promise.all(rooms.filter(r=>r.connection.isOpen).map(r=>r.leave().catch(()=>{})));await delay(200);}

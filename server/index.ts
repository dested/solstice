...
if (process.env.PUBLIC_ADDRESS && !redis)
  throw new Error("PUBLIC_ADDRESS requires REDIS_URL for shared matchmaking");
...
const game = new Server({
  transport: new BunWebSockets({
    ...
    beforeUpgrade: (_request, context) =>
      allowedOrigin(context.headers)
        ? undefined
        : new Response("Forbidden", { status: 403 }),
  }),
  ...(redis
    ? { presence: new RedisPresence(redis), driver: new RedisDriver(redis) }
    : {}),
  ...
  express: (app) => {
    ...
    app.get("/api/status", async (_req, res) => {
      try {
        const rooms = await matchMaker.query({ name: "universe" });
        res.json({
          players: rooms.reduce((n, r) => n + (r.metadata?.humans ?? 0), 0),
          worlds: rooms.length,
          capacity: 64,
        });
      } catch {
    ...
    app.get("/api/bot-worlds", async (req, res) => {
      if (!validBotToken(req.headers.authorization?.replace(/^Bearer /, ""))) {
    ...
    app.use(async (req, res) => {
      ...
      if (!(await file.exists())) {
        res
          .status(200)
          .send(
            "Solstice server is running. Open http://localhost:5173 for development.",
          );
        return;
      }
...
game.onBeforeShutdown(async () => {
  ...
  const seconds = Number(
    process.env.DRAIN_SECONDS ??
      (process.env.NODE_ENV === "production" ? "60" : "0"),
  );
  for (const room of liveRooms) {
    await room.lock();
    room.broadcast("shutdown", { seconds });
  }
  if (seconds > 0 && liveRooms.size)
    await new Promise((resolve) => setTimeout(resolve, seconds * 1000));
});
await game.listen(port, "0.0.0.0");
console.log(
  `Solstice game server: http://localhost:${port} (${redis ? "Redis matchmaking" : "single process"})`,
);

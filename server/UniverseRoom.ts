...
export function validBotToken(token: unknown) {
  const expected =
    ...
    (process.env.NODE_ENV !== "production" ? "dev-bot-secret" : "");
  return (
    !!expected &&
    typeof token === "string" &&
    token.length <= 256 &&
...
export class UniverseRoom extends Room {
  ...
  onCreate() {
    ...
    this.onMessage("move", (client, msg: MoveOrder) => {
      ...
      seat.tokens = Math.min(8, seat.tokens + (now - seat.last) * 0.004);
      seat.last = now;
      if (seat.tokens < 1) return;
      seat.tokens--;
      const count = this.sim.order(seat.player, msg);
      client.send("ordered", { count });
    });
    this.onMessage("view", (client, v: Viewport) => {
      const seat = this.seats.get(client.sessionId);
      if (!seat || !v || ![v.x, v.y, v.width, v.height].every(Number.isFinite))
        return;
      seat.view = {
        x: clamp(v.x, 0, WORLD_SIZE),
        y: clamp(v.y, 0, WORLD_SIZE),
        width: clamp(v.width, 300, WORLD_SIZE * 1.5),
        height: clamp(v.height, 300, WORLD_SIZE * 1.5),
      };
    });
    this.onMessage("respawn", (client) => {
      const seat = this.seats.get(client.sessionId);
      const p = seat && this.sim.players.get(seat.player);
      this.sim.recount();
      if (p?.eliminated) {
        this.sim.spawn(p);
        this.welcome(client);
    ...
    this.setSimulationInterval(() => {
      ...
      const now = performance.now();
      accumulator += Math.min((now - lastTick) / 1000, 0.2);
      lastTick = now;
      while (accumulator >= TICK) {
        this.sim.step(TICK);
        this.pending.push(...this.sim.events);
        accumulator -= TICK;
      }
      this.frame++;
  ...
  onAuth(_client: Client, options: unknown, context: AuthContext) {
    if (!allowedOrigin(context.headers))
    ...
    if (
      ...
      throw new ServerError(400, "Invalid name");
    if (
      "bot" in options &&
      options.bot &&
      (!("token" in options) || !validBotToken(options.token))
    )
      throw new ServerError(403, "Invalid bot credential");
    return true;
  }
  onJoin(
    client: Client,
    options: { name?: string; bot?: boolean; slot?: number },
  ) {
    const humans = [...this.seats.values()].filter(
      (s) => s.botSlot === undefined,
    ).length;
    if (
      options.bot &&
      (!Number.isInteger(options.slot) ||
        options.slot! < 0 ||
        options.slot! >= Math.max(0, botTarget - humans) ||
        !humans ||
        [...this.seats.values()].some((s) => s.botSlot === options.slot))
    )
      throw new ServerError(409, "Bot slot unavailable");
    // Make room for humans first; bots never consume the last human seat.
    if (this.sim.players.size >= MAX_PLAYERS) {
      const expendable = [...this.sim.players.values()].find(
        (p) => p.bot || !p.connected,
      );
    ...
    this.seats.set(client.sessionId, {
      player: p.id,
      view: { x: home.x, y: home.y, width: 1700, height: 1200 },
      tokens: 8,
      last: performance.now(),
      botSlot: options.bot ? options.slot : undefined,
    });
    this.reconcileBots();
    this.welcome(client);
  }
  private reconcileBots() {
    ...
    for (const client of [...this.clients]) {
      const seat = this.seats.get(client.sessionId);
      if (seat?.botSlot !== undefined && seat.botSlot >= target) client.leave();
    }
    void this.setMetadata({
      humans,
      botSlots: [...this.seats.values()].flatMap((s) =>
        s.botSlot === undefined ? [] : [s.botSlot],
      ),
      botTarget,
    });
  }
  welcome(client: Client) {
    ...
    client.send("world", this.world());
    client.sendBytes("units", encodeUnits(this.sim.visible(p.id, seat.view)));
  }
  world() {
    this.sim.recount();
    return {
      ...
      players: [...this.sim.players.values()],
      room: this.roomId,
      unitCount: this.sim.units.size,
    };
  }
  private sendMetadata() {
    this.broadcast("world", this.world());
  }
  private stream() {
    const index = this.sim.snapshotIndex();
    for (const client of this.clients) {
      const seat = this.seats.get(client.sessionId);
      if (!seat) continue;
      client.sendBytes(
        "units",
        encodeUnits(this.sim.visible(seat.player, seat.view, 6000, index)),
      );
  ...
  onDrop(client: Client) {
    const seat = this.seats.get(client.sessionId);
    if (seat && seat.botSlot === undefined)

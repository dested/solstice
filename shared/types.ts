...
export function factionColor(id: number) {
  return id === 0
    ? "#587188"
    : id <= PALETTE.length
      ? PALETTE[id - 1]
      : `hsl(${Math.round(id * 137.508) % 360}, ${65 + (id % 3) * 8}%, ${65 + (id % 4) * 4}%)`;
}
...
export function encodeUnits(units: Iterable<Unit>): Uint8Array {
  ...
  const buffer = new ArrayBuffer(list.length * 16);
  const v = new DataView(buffer);
  for (let i = 0; i < list.length; i++) {
    const u = list[i];
    const n = i * 16;
    v.setUint32(n, u.id, true);
    v.setFloat32(n + 4, u.x, true);
    v.setFloat32(n + 8, u.y, true);
    v.setUint16(n + 12, u.owner, true);
    v.setUint16(n + 14, u.moving ? 1 : 0, true);
  }
  return new Uint8Array(buffer);
}
export function decodeUnits(
  bytes: Uint8Array,
): Pick<Unit, "id" | "owner" | "x" | "y" | "moving">[] {
  if (bytes.byteLength % 16) return [];
  const v = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  const result = [];
  for (let n = 0; n < bytes.byteLength; n += 16)
    result.push({
      id: v.getUint32(n, true),
      x: v.getFloat32(n + 4, true),
      y: v.getFloat32(n + 8, true),
      owner: v.getUint16(n + 12, true),
      moving: !!v.getUint16(n + 14, true),
    });
  return result;
}

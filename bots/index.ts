...
const addressMap = JSON.parse(
  process.env.BOT_PUBLIC_ADDRESS_MAP ?? "{}",
...
async function joinBot(listing: Listing, slot: number) {
  const key = `${listing.roomId}:${slot}`;
  if (bots.has(key)) return;
  try {
    const room = await new Client(endpoint, {
      urlBuilder: (url) => botRoute(url, addressMap),
    }).joinById(listing.roomId, {
    ...
    const bot: Bot = {
      ...
      id: 0,
      units: [],
      nextThink: performance.now() + 600 + Math.random() * 800,
    };
    bots.set(key, bot);
    ...
    room.onMessage("world", (w: WorldMeta) => {
      bot.world = w;
    });
    room.onMessage("units", (bytes: Uint8Array) => {
      bot.units = decodeUnits(bytes);
    });
    ...
    room.onLeave(() => bots.delete(key));
    room.onError(() => {});
  } catch (error) {
    const code = (error as { code?: number }).code;
    if (code !== 409 && code !== 421 && code !== 524 && code !== 506)
      console.warn(
        "Bot join failed:",
...
async function poll() {
  ...
  working = true;
  try {
    const response = await fetch(`${endpoint}/api/bot-worlds`, {
      headers: { Authorization: `Bearer ${secret}` },
      signal: AbortSignal.timeout(5000),
    });
    if (!response.ok)
      throw new Error(`World discovery returned ${response.status}`);
    const worlds = (await response.json()) as Listing[];
    lastPoll = Date.now();
    worlds.sort(() => Math.random() - 0.5);
    for (const world of worlds) {
      const desired = world.humans
        ? Math.max(0, world.botTarget - world.humans)
        : 0;
      for (
        let slot = 0;
        slot < desired && bots.size < capacity && !stopping;
        slot++
      )
        if (!world.botSlots?.includes(slot)) await joinBot(world, slot);
    }
  } catch (error) {
    console.warn(
      "Bot discovery:",
...
const health = Bun.serve({
  ...
  fetch: (req) =>
    new URL(req.url).pathname === "/healthz"
      ? Response.json(
          {
            ok: !stopping && Date.now() - lastPoll < 15000,
            bots: bots.size,
            capacity,
            orders,
            lastPoll,
          },
          { status: !stopping && Date.now() - lastPoll < 15000 ? 200 : 503 },
        )
...
async function stop() {
  ...
  await Promise.all(
    [...bots.values()]
      .filter((b) => b.room.connection.isOpen)
      .map((b) => b.room.leave().catch(() => {})),
  );
  health.stop();
  process.exit(0);
}
...
console.log(
  `Solstice bot worker: ${endpoint}; capacity ${capacity}; health :${health.port}`,
);
await poll();
